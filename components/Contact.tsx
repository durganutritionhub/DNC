...
export function Contact() {
  ...
  const handleWhatsAppSubmit = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const programs = selectedPrograms.join(', ');
    ...
    let text = `Hi Coach Durga / DNC Wellness!\nI would like to enquire about your nutrition programs.\n\n`;
    text += `👤 *Name:* ${name}\n`;
    if (phone) text += `📱 *Phone:* ${phone}\n`;
    if (email) text += `📧 *Email:* ${email}\n`;
    text += `🎯 *Interested Programs:* ${programs}\n`;
    if (message) text += `💬 *Message:* ${message}\n`;

    const encodedText = encodeURIComponent(text);
    const whatsappUrl = `https://wa.me/917981566705?text=${encodedText}`;
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5 sm:gap-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
                  <button
                    ...
                    className="w-full py-3 sm:py-3.5 px-4 rounded-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-md hover:shadow-lg transition-all active:scale-[0.99] cursor-pointer"
                  >
                  ...
                  <Button 
                    type="submit" 
                    variant="outline" 
                    className="w-full py-3 sm:py-3.5 text-xs sm:text-sm font-semibold border-primary-green/40 hover:bg-primary-green hover:text-white"
                    disabled={status === 'submitting'}
                  >
                    {status === 'submitting' ? t('form_submitting') : 'Send via Email'}
                  </Button>
                </div>
              </form>
            )}
